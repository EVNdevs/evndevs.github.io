...
| Method | Does |
| :--- | :--- |
| `calibrate()`, `calibrate(wait=True)` | the self-calibration of this port: about 4.5 s with the shaft free to turn (it moves up to about a turn each way and ends near where it started). Returns `(b0, tau_ms, v_break_mv, v_f_mv)` |
