...
stopping, the measurements, the speed and acceleration settings, and the gyro loop over an
evn.Pose. The moves go out and come back, so the robot ends near where it started. Distances are
...
"""
from evn import Motor, Direction, DriveBase, IMU, Pose, Stop, StopWatch, wait, stop_all
...
# With an evn.Pose on the same two motors (and the IMU), every move is corrected as it goes, so
...
robot.stop()
robot.close()                                    # coasts both wheels and releases them
pose.close()
imu.close()
stop_all()
left.close()
